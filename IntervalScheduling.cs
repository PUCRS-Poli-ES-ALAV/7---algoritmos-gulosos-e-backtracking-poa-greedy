...
{
    public static List<int> SDMGreedy(List<int> s, List<int> f)
    {
        int n = s.Count;
    ...
    {
        Console.WriteLine("Digite o número de intervalos:");
        int n = int.Parse(Console.ReadLine() ?? "0");

        List<int> s = new List<int>();
        List<int> f = new List<int>();

        Console.WriteLine("Digite os inícios dos intervalos:");
        for (int i = 0; i < n; i++)
        {
            s.Add(int.Parse(Console.ReadLine() ?? "0"));
        }

        Console.WriteLine("Digite os términos dos intervalos:");
        for (int i = 0; i < n; i++)
        {
            f.Add(int.Parse(Console.ReadLine() ?? "0"));
        }

        List<int> result = SDMGreedy(s, f);
        Console.WriteLine("Índices dos intervalos:");
